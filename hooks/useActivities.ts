...
import { Activity, Status, STATUS_TARGET } from '@/types';
...
export function useActivities(date?: string) {
    ...
    async function toggleStatus(id: string, targetStatus: Status) {
        // Optimistic update
        mutate(
            (prev) => prev?.map(a => a.id === id ? { ...a, status: targetStatus } : a),
            false
        )
